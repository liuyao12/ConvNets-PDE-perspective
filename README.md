...
* This started out as a response to [ConvNext](https://github.com/facebookresearch/ConvNeXt), first as a Twitter thread, then expanded on Weights & Biases. In my (biased) opinion, if anything is to be called the "ConvNets of the 2020s", it would be those that are designed from the PDE perspective. 
* Everyone is invited to **open a discussion**, to offer criticisms, ask for clarification, suggest and develop new ideas and experiments, a la the Polymath project — an open collaboration that works well for iterations of "small" improvements, as opposed to deep conceptual advancement; and of course to PR any code and results that you would like to share. The hope is to get researchers from all over the world to collaborate, to move the field forward, without the inefficient cycle of published papers that repeat many of the same things over. Unlike the Polymath projects, there doesn't appear to be a clearly defined goal.
